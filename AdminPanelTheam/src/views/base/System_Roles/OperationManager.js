...
const OperationManager=()=> {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/CreateOperationManager").then((result) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(
          `http://localhost:5000/CreateOperationManager/${id}`,
          {
            method: "DELETE",
          }
        );
...
<CButton
  onClick={() => navigate("/base/System_Roles/CreateOperationManager")}
  color="primary"
  className="me-md-2"
>
