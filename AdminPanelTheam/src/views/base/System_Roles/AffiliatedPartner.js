...
const AffiliatedPartner=()=> {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/CreateAffiliatedPartner").then((result) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(
          `http://localhost:5000/CreateAffiliatedPartner/${id}`,
          {
            method: "DELETE",
          }
        );
...
<CButton
  onClick={() => navigate("/base/System_Roles/CreateAffiliatedPartner")}
  color="primary"
  className="me-md-2"
>
